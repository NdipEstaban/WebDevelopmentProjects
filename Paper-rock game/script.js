...
var roundNumber;
var trackRounds = 0;
let compNames = ['Jarvis', 'Edith', 'Mavin', 'Gabriel', 'Roxxy'];
...
function login(){                                                                                                                                                                                                                                                                                                       
    ...
    if(/\w{2,10}/.test(player) == false || /\d/.test(roundNumber) == false ){
        alert("Please enter a valid name of atleast 2 letters, don't forget the number of rounds.");
        return 0;
...
function chose(n){
    ...
    if(roundNumber == 0){
        ...
        document.getElementById('final-computer-score').innerText = trackCompScore;
        document.getElementById('final-player-score').innerText = trackPlayerScore;
        document.getElementById('final-computer-name').innerText = chosenCompName;
        document.getElementById('final-player-name').innerText = player;
        document.getElementById('draws').innerText = trackRounds - (trackCompScore + trackPlayerScore);
